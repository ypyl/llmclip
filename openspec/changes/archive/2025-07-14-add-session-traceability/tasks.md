...
- [x] 5.1 Run `main.ahk`, send a message to LLM, verify `sessions/` directory is created and contains a JSONL file with the expected structure *(manual verification)*
- [x] 5.2 Trigger a tool call (e.g., ask about a file on disk), verify both round-trips are logged as separate lines *(manual verification)*
- [x] 5.3 Cancel a request mid-flight, verify no line is written for the cancelled call *(manual verification)*
- [x] 5.4 Inspect a log line, verify no API key appears in `request` or `response_raw` *(manual verification)*
